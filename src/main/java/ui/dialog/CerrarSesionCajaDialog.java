...
public class CerrarSesionCajaDialog extends JDialog {
	...
	private void cargarCajasOcupadas() {
		...
		appServices.sesionCajaService.getEstadoCajas().stream().filter(CajaEstadoDTO::isOcupada).forEach(caja -> {
	...
	private JToggleButton crearBotonCaja(CajaEstadoDTO caja) {
		...
		String texto = "<html><center><b>" + caja.getNombreCaja() + "</b><br/>" + "Ocupada<br/>("
	...
	private PinDialogResult abrirDialogoPin() {
		PinDialog dialog = new PinDialog((JFrame) SwingUtilities.getWindowAncestor(this),
				PinDialog.PinDialogMode.LOGIN_RAPIDO, "Empleado sesión");
	...
	private void validarEmpleadoSesion(String codigo) {
		try {
			if (sesionAbierta == null) {
				sesionAbierta = appServices.sesionCajaService.getSesionAbiertaOrThrow(cajaSeleccionada.getIdCaja());
			}
			...
			JOptionPane.showMessageDialog(this, ex.getMessage(), "Validación de empleado", JOptionPane.ERROR_MESSAGE);
		}
	}

	private void mostrarEmpleadoValidado() {
		lblEmpleadoValidado.setText(
				"✔ Empleado validado: " + empleadoValidado.getNombre() + " (" + empleadoValidado.getUsuario() + ")");
	...
	private void abrirDialogoConteo() {
		...
		importeEsperado = appServices.sesionCajaService.calcularEfectivoEsperado(sesionAbierta.getIdSesion());
	...
	private String generarTicketCierre() {
	    ...
	    sb.append("Importe inicial:   ")
	    .append(MONEY.format(sesionAbierta.getImporteInicial()))
	    .append(" €\n");

	  sb.append("Importe esperado:  ")
	    .append(MONEY.format(importeEsperado))
	    .append(" €\n");

	  sb.append("Importe contado:   ")
	    .append(MONEY.format(importeContado))
	    .append(" €\n");

	  sb.append("----------------------------\n");

	  sb.append("DESFASE:           ")
	    .append(MONEY.format(desfase))
	    .append(" €\n");
	...
	private void confirmarCierreCaja() {
	    ...
	    try {
	        ...
	        dispose(); // volvemos al frame anterior
