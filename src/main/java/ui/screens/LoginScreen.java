...
public class LoginScreen extends JFrame {
	...
	public LoginScreen(AppServices services, int terminal) {
	...
	private void initUI() {
		...
		panelCentral.setBorder(BorderFactory.createCompoundBorder(new LineBorder(Color.BLACK, 3, true),
		...
		panelBotonesRecordados.setBorder(BorderFactory.createTitledBorder("Partner Sign-In"));
		...
		btnSignIn.addActionListener(e -> {
			...
			PinDialog dialog = new PinDialog(this, PinDialog.PinDialogMode.LOGIN_COMPLETO, null);
			...
			try {
				...
				services.usuarioRecordadoService.registrarAccesoExitoso(usuarioLogueado, idTerminal);
				continuarPostLogin();
	...
	private void continuarPostLogin() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, ex.getMessage(), "Error post-login", JOptionPane.ERROR_MESSAGE);
		}
	}

	private void abrirAdminDashboard() {

	    AdminDashboardFrame dash =
	...
	private void iniciarReloj(JLabel label) {
		...
		Timer timer = new Timer(1000, e -> {
			...
			label.setText("<html><div style='text-align: right;'>" + ahora.format(fechaFormato) + "<br>"
					+ "<span style='font-size: 20px; color: white;'>" + ahora.format(horaFormato)
					+ "</span></div></html>");
		});
		timer.start();
	}
	...
	private void cargarBotonesRecordados() {
		...
		List<UsuarioRecordado> usuarios = services.usuarioRecordadoService.getUsuariosRecordadosDelTerminal(idTerminal);
	...
	private JButton crearBotonUsuarioRecordado(UsuarioRecordado ur) {
		...
		btn.addActionListener(e -> {
			...
			UsuarioRecordado usuario = (UsuarioRecordado) ((JButton) e.getSource())
					.getClientProperty("usuarioRecordado");

			// 2. Abrir PinDialog en modo LOGIN_RAPIDO
			PinDialog dialog = new PinDialog(this, PinDialog.PinDialogMode.LOGIN_RAPIDO, usuario.getNombreBoton());
			...
			try {
				...
				Usuario usuarioLogueado = services.authService.loginRapido(usuario.getIdUsuario(), result.getPin());
				...
				JOptionPane.showMessageDialog(this, ex.getMessage(), "Error de autenticación",
