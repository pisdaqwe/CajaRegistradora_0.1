...
/**
 * Dashboard de Manager (ADMIN / TECNICO).
...
public class AdminDashboardFrame extends BaseTpvFrame {
    ...
    public AdminDashboardFrame(Runnable onLogoutNavigate) {
        super("Panel de Administración", onLogoutNavigate);
        this.onLogoutNavigate = onLogoutNavigate;

        // Init services
        this.sesionCajaService = new SesionCajaService(
                new CajaDao(),
                new SesionCajaDao()
        );
    ...
    private void aplicarPermisos(JButton btnEmpleados, JButton btnConfig) {
        ...
        String rol = (u.getRol() != null && u.getRol().getNombre() != null)
                ? u.getRol().getNombre().toUpperCase()
                : "";
    ...
    private void onInformes() {
        ...
        InformesMenuFrame frame =
                new InformesMenuFrame(onLogoutNavigate, () -> this.setVisible(true));
        frame.setVisible(true);
    }

    private void onEmpleados() {
        this.setVisible(false);
        EmpleadoMenuFrame frame =
                new EmpleadoMenuFrame(onLogoutNavigate, () -> this.setVisible(true));
        frame.setVisible(true);
    }

    private void onConfig() {
        this.setVisible(false);
        ConfiguracionMenuFrame frame =
                new ConfiguracionMenuFrame(onLogoutNavigate, () -> this.setVisible(true));
        frame.setVisible(true);
    }

    private void onAuditoria() {
        this.setVisible(false);
        AuditoriaMenuFrame frame =
                new AuditoriaMenuFrame(onLogoutNavigate, () -> this.setVisible(true));
        frame.setVisible(true);
    }

    private void onHerramientas() {
        this.setVisible(false);
        SistemaMenuFrame frame =
                new SistemaMenuFrame(onLogoutNavigate, () -> this.setVisible(true));
        frame.setVisible(true);
    }
