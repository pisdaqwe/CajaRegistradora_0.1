...
public class SesionCajaDao {
	...
	public List<CajaEstadoDTO> findEstadoCajas() {
		...
		try (Connection conn = DbPool.getConnection();
			...
			while (rs.next()) {
				...
				boolean operativa = "OPERATIVA".equals(rs.getString("estado_caja"));

				resultado.add(new CajaEstadoDTO(rs.getInt("id_caja"), rs.getString("nombre_caja"),
						rs.getBoolean("ocupada"), rs.getString("empleado"), operativa));
			}
	...
	public boolean existeSesionAbiertaPorUsuario(int idUsuario) {
		...
		try (Connection conn = DbPool.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			throw new RuntimeException("Error comprobando sesión abierta para usuario " + idUsuario, e);
		}
	}
